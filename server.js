...
async function start() {
  ...
  passport.deserializeUser((_id, done) => {
    Account.findById(_id)
      .select("name tel isAccountValidated")
      .exec((err, account) => {
        if (err) {
          return done(err);
        }
    ...
    new LocalStrategy(
      ...
      async (tel, pwd, done) => {
        Account.findOne({ tel: tel })
          .select("_id name isAccountValidated tel pwd")
          .exec((err, account) => {
            if (err) {
              return done(err);
            }
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (
        account.idUri !== req.params.id &&
        account.wcardUri !== req.params.id &&
        account.codcUri !== req.params.id
      )
