<template>
    ...
    <div class="columns is-centered mt-6">
      <div class="column is-two-thirds-tablet is-half-desktop">
        <h2 class="has-text-centered title has-text-primary mb-3">Changement de mot de passe</h2>
          ...
          <b-notification
            ...
            has-icon
            icon="check"
            aria-close-label="Close notification">Code envoyé avec success. Veuillez consulter votre boite de reception mail.</b-notification>
          <div class="field" v-if="activeForm === 'email'">
            <label class="label help" for="email">Address e-mail:</label>
            <input  v-model="email" id="email" class="input" type="email" placeholder="exemple@exemple.com" />
            <button @click="sendEmail" class="button mt-2 is-fullwidth is-primary" type="submit">
              M'envoyer un code de récupération
            </button>
          </div>
          <div class="field" v-if="activeForm === 'code'">
            ...
            <button @click="sendCode" class="button mt-2 is-fullwidth is-primary" type="submit">
              Envoyer
            </button>
          </div>
          <div class="field" v-if="activeForm === 'pass'">
            ...
            <input v-model="confirmedPWD" id="confirmedPWD" class="input" type="password" placeholder="********" />
            <button @click="sendPass" class='button mt-2 is-fullwidth is-primary' type="submit">
    ...
    async sendPass() {
      ...
      try {
        ...
        const res = await this.$axios.$post("/api/auth/resetpass", {
          data: { pwd: this.pwd, confirmedPWD: this.confirmedPWD, email: this.email }
        });
        if(res.message){
          const csrf = await this.$axios.$get("/api/auth/csrf");
          this.$axios.setHeader("XSRF-TOKEN", csrf.token);
