  ...
  Account.findOne({ tel })
    ...
    .exec((err, doc) => {
      ...
      if (doc)
        return res
          .status(400)
          .json({ err: "Numéro déjà utilisé. Veuillez vous connecter." });
      const fieldsValidationResult = validateAccountInformations(req.body.data);
      if (fieldsValidationResult)
        return res.status(400).json({ err: fieldsValidationResult });
      ...
      bcrypt.hash(
        ...
        (bcrypt_err, hashedPassword) => {
          if (bcrypt_err)
            return res
              .status(500)
              .json({ err: "Erreur survenue. Veuillez reéssayer." });
